...
const val ENDPOINT = "api2"
const val PARAMS = "ar=1&k=6LerBOgUAAAAAKPg6vsAFPTN66Woz-jBClxdQU-o&co=aHR0cHM6Ly9yb2JlcnRzc3BhY2VpbmR1c3RyaWVzLmNvbTo0NDM.&hl=zh-CN&v=vP4jQKq0YJFzU6e21-BGy3GP&size=invisible&cb=lam2123evvtl"

const val K = "6LerBOgUAAAAAKPg6vsAFPTN66Woz-jBClxdQU-o"
const val CO = "aHR0cHM6Ly9yb2JlcnRzc3BhY2VpbmR1c3RyaWVzLmNvbTo0NDM."
const val V = "vP4jQKq0YJFzU6e21-BGy3GP"


const val BASE = "https://recaptcha.net/recaptcha/"
const val CONTENT_TYPE = "application/x-www-form-urlencoded"

const val firstStagePattern = "\"recaptcha-token\" value=\"(.*?)\""
const val secondStagePattern = "\"rresp\",\"(.*?)\""

class RecaptchaV3 {
    companion object {
        fun getPostData(token: String): String {
        ...
        suspend fun getFirstStageToken(): String = withContext(Dispatchers.IO) {
            val url = "$BASE$ENDPOINT/anchor?$PARAMS"
            val request = Request.Builder()
                .url(url)
                .header("Content-Type", CONTENT_TYPE)
                .addHeader("User-Agent", "Mozilla/5.0 (Linux; Android 11; Pixel 3a) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.210 Mobile Safari/537.36")
                .build()
            val response = cirnoClient.newCall(request).execute()
            val body = response.body?.string()
            val pattern = Regex(firstStagePattern)
            val result = pattern.find(body!!)
            return@withContext result!!.groupValues[1]
        }

        suspend fun getSecondStageToken(firstStageToken: String): String =
            withContext(Dispatchers.IO) {
                val url = "$BASE$ENDPOINT/reload?k=$K"
                val postData = getPostData(firstStageToken)
                val request = Request.Builder()
                    .url(url)
                    .header("Content-Type", CONTENT_TYPE)
                    .addHeader("User-Agent", "Mozilla/5.0 (Linux; Android 11; Pixel 3a) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.210 Mobile Safari/537.36")
                ...
                val response = cirnoClient.newCall(request).execute()
                val body = response.body?.string()
                val pattern = Regex(secondStagePattern)
                val result = pattern.find(body!!)
                return@withContext result!!.groupValues[1]
            }

        suspend fun getRecaptchaToken(): String {
            val firstStageToken = getFirstStageToken()
            return getSecondStageToken(firstStageToken)
        }
    }
}